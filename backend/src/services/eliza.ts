...
export class ElizaService {
   ...
   */
  private async evaluateContributions(
    ctx: Context,
    weekAgo: number
  ): Promise<string> {
    const message = ctx.message
  ...
  public async generateResponse(ctx: Context) {
    ...
    if (ctx.message.reply_to_message && messageText.includes("送りたい")) {
      ...
      if (confirmationData.action === "transfer") {
        ...
        const extractedReceiver = this.extractTagValue(
          suggestionText,
          "receiverUserId"
        )
    ...
    if (messageText.toLowerCase().startsWith("/suggest")) {
      ...
      const aggregatedText1 = elizaMemories
      ...
      const assistCreditTokenId = this.extractTagValue(
        proposalXml,
        "assistCreditTokenId"
      )
      const extractedReceiver = this.extractTagValue(
